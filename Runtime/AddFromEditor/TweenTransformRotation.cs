...
namespace U.Motion
{
    public class TweenTransformRotation : MonoBehaviour
    {
        [Space(10)]
        public Transform t;
        ...
        private void AddTween()
        {
            ...
            if (x && !y && !z)
            {
                animator = t.TweenRotationX(tweenA);
            }
            else if (!x && y && !z)
            {
                animator = t.TweenRotationY(tweenA);
            }
            else if (!x && !y && z)
            {
                animator = t.TweenRotationZ(tweenA);
            }

            else if (x && y && !z)
            {
                animator = t.TweenRotationXY(tweenAB);
            }
            else if (x && !y && z)
            {
                animator = t.TweenRotationXZ(tweenAB);
            }
            else if (!x && y && z)
            {
                animator = t.TweenRotationYZ(tweenAB);
            }

            else if (x && y && z)
            {
                animator = t.TweenRotationXYZ(tweenABC);
            }
