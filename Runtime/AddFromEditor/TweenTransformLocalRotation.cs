...
namespace U.Motion
{
    public class TweenTransformLocalRotation : MonoBehaviour
    {
        [Space(10)]
        public Transform t;
        ...
        private void AddTween()
        {
            ...
            if (x && !y && !z)
            {
                animator = t.TweenLocalRotationX(tweenA);
            }
            else if (!x && y && !z)
            {
                animator = t.TweenLocalRotationY(tweenA);
            }
            else if (!x && !y && z)
            {
                animator = t.TweenLocalRotationZ(tweenA);
            }

            else if (x && y && !z)
            {
                animator = t.TweenLocalRotationXY(tweenAB);
            }
            else if (x && !y && z)
            {
                animator = t.TweenLocalRotationXZ(tweenAB);
            }
            else if (!x && y && z)
            {
                animator = t.TweenLocalRotationYZ(tweenAB);
            }

            else if (x && y && z)
            {
                animator = t.TweenLocalRotationXYZ(tweenABC);
            }
